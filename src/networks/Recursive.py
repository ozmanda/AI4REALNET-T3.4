...
class RecursiveLayer(nn.Module): 
    def __init__(self, n_features, output_size, hidden_size, general_size):
        super().__init__()
        ...
        self.encode_general = nn.Sequential(nn.Linear(general_size, 2 * hidden_size),
                                            nn.ReLU(inplace=True),
    ...
    def forward(self, state: Tensor, general_size: int): 
        ...
        features: Tensor = state[:, general_size:]
        ...
        general: Tensor =  state[:, :general_size]
