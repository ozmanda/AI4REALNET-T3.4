...
def value_loss(state_values: Tensor, new_state_values: Tensor, reward: Tensor, done: Tensor, gamma: float, actual_len: int = 1) -> Tensor:
    """
    ...
    """
    expected_state_values = (new_state_values.detach() * gamma ** actual_len * (1 - done)) + reward
...
def value_loss_with_IS(state_values: Tensor, new_state_values: Tensor, new_log_prob: Tensor, old_log_prob: Tensor, reward: Tensor, done: Tensor, gamma: float, actual_len: int = 1):
    ...
    value_loss = (F.mse_loss(state_values, expected_state_values, reduction="none") * importance_sample_fix).mean()
    return value_loss
